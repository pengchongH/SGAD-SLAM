...
def create_point_cloud(image: np.ndarray, depth: np.ndarray, intrinsics: np.ndarray, pose: np.ndarray) -> np.ndarray:
    ...
    x = (u - intrinsics[0, 2] + 0.5) * depth / intrinsics[0, 0]
    y = (v - intrinsics[1, 2] + 0.5) * depth / intrinsics[1, 1]
    z = depth
...
def create_point_cloud_torch(image: torch.Tensor, depth: torch.Tensor, intrinsics: torch.Tensor, pose: torch.Tensor) -> torch.Tensor:
    """
    Creates a point cloud from an image, depth map, camera intrinsics, and pose.
    ...
    height, width = depth.shape
    # Create a mesh grid of pixel coordinates
    u, v = torch.meshgrid(torch.arange(width).cuda().float(), torch.arange(height).cuda().float(), indexing='xy')
    # print(u.shape, v.shape)
    # print(depth.shape)
    # Convert pixel coordinates to camera coordinates
    x = (u - intrinsics[0, 2] + 0.5).mul(depth) / intrinsics[0, 0]
    y = (v - intrinsics[1, 2] + 0.5).mul(depth) / intrinsics[1, 1]
    z = depth
    # Stack the coordinates together
    points = torch.stack((x, y, z, torch.ones_like(z)), dim=-1)
    # Reshape the coordinates for matrix multiplication
    points = points.view(-1, 4)
    # Transform points to world coordinates
    posed_points = torch.matmul(pose, points.T)
    posed_points = posed_points.T[:, :3]
    # Flatten the image to get colors for each point
    colors = image.view(-1, 3)
    # Concatenate posed points with their corresponding color
    point_cloud = torch.cat((posed_points, colors), dim=-1)

    return point_cloud


def get_init_gs_scales(image: np.ndarray, depth: np.ndarray, intrinsics: np.ndarray, pose: np.ndarray) -> np.ndarray:
    """
    Creates a point cloud from an image, depth map, camera intrinsics, and pose.
    ...
    FX = intrinsics[0, 0]
    FY = intrinsics[1, 1]
    scale_gaussian = z / ((FX + FY)/2)
    sq_scale_gaussian = scale_gaussian**2
    sq_scale_gaussian = sq_scale_gaussian.reshape(-1, 1)

    return sq_scale_gaussian