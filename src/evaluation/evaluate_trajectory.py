...
def align_trajectories(t_pred: np.ndarray, t_gt: np.ndarray):
    ...
    R, t, trans_error = align(t_align, np.matrix(t_gt).transpose())
    t_align = R * t_align + t
    t_align = np.asarray(t_align).T
    return t_align, trans_error
...
def evaluate_trajectory(estimated_poses: np.ndarray, gt_poses: np.ndarray, output_path: Path):
    ...
    estimated_t_aligned, trans_error = align_trajectories(estimated_t, gt_t)
    ate_aligned = trans_error.mean()



    ax = plot_2d(estimated_t_aligned,
                 label=f"ate-rsme (aligned): {round(ate_aligned * 100, 2)} cm", color="lightskyblue")
    ax = plot_2d(gt_t, ax, label="GT", color="green")
    ax.legend()
    plt.savefig(str(output_path / "eval_trajectory.png"), dpi=300)
    plt.close()
    print(
        f"ATE-RMSE: {ate_aligned * 100:.2f} cm")
