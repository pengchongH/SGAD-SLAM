...
class Logger(object):
    ...
    def vis_tracking_iteration(self, frame_id, iter, color, depth, gt_color, gt_depth, tracking_mask, final_rendered_color=None, final_rendered_depth=None, seeding_mask=None, interval=10) -> None:
        """
        Visualization of depth, color images and save to file.
        ...
        color_np = color.detach().cpu().numpy()
        depth_residual = np.abs(gt_depth_np - depth_np)
        depth_residual = depth_residual * tracking_mask
        ...
        color_residual = np.abs(gt_color_np - color_np)
        color_residual = color_residual * tracking_mask
        color_residual[np.squeeze(gt_depth_np == 0.0)] = 0.0
        ...
        axs[1, 3].imshow(tracking_mask, cmap="gray")
        axs[1, 3].set_title('Tracking Mask', fontsize=16)
        axs[1, 3].set_xticks([])
        axs[1, 3].set_yticks([])

        if final_rendered_color is not None:
            final_rendered_color = torch.round(final_rendered_color * 255.0) / 255.0
            final_rendered_color = final_rendered_color.detach().cpu().numpy()
            axs[0, 3].imshow(final_rendered_color, cmap="plasma")
            axs[0, 3].set_title('Final Rendered keypoint RGB', fontsize=16)
            axs[0, 3].set_xticks([])
            axs[0, 3].set_yticks([])
        if final_rendered_depth is not None:
            final_rendered_depth = final_rendered_depth.detach().cpu().numpy()
            axs[0, 3].imshow(final_rendered_depth, cmap="jet", vmin=0, vmax=6)
            axs[0, 3].set_title('Final Rendered keypoint Depth', fontsize=16)
            axs[0, 3].set_xticks([])
            axs[0, 3].set_yticks([])
        ...
        fig.tight_layout()
        plt.subplots_adjust(top=0.90)  # Adjust top margin
        fig_name = str(self.output_path / "tracking_vis" / f'{frame_id:04d}_{iter:04d}.jpg')
        fig_title = f"Tracker Color/Depth at frame {frame_id:04d} iters {iter:04d}"
        plt.suptitle(fig_title, y=0.98, fontsize=20)
        plt.savefig(fig_name, dpi=250, bbox_inches='tight')
        plt.clf()
        plt.close()
        if self.use_wandb:
            log_title = "Tracking_vis/" + f'{frame_id:04d}_{iter:04d}'
            wandb.log({log_title: [wandb.Image(fig_name)]})
        print(f"Saved rendering vis of color/depth at {frame_id:04d}_{iter:04d}.jpg")