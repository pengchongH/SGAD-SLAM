...
class GaussianModel:
    def __init__(self, sh_degree: int = 3, isotropic=True): #True False
    ...
    def add_points(self, pcd: o3d.geometry.PointCloud, scales_gs, estimate_c2w, global_scale_init=True):
        ...
        scales_gs = torch.tensor(scales_gs).float().cuda().squeeze()
        scales = torch.log(torch.sqrt(scales_gs))[..., None].repeat(1, 3)
    ...
    def fix_setup(self, exposure_ab=None):
        ...
        if exposure_ab is not None:
            params.extend([
                {"params": [exposure_ab[0]], "lr": 0, "name": "exposure_a"},
                {"params": [exposure_ab[1]], "lr": 0, "name": "exposure_b"}]
            )

        self.fix_optimizer = torch.optim.Adam(params, lr=0.0, eps=1e-15)
    ...
    def prune_points_5(self):
        optimizable_tensors = self._prune_optimizer_5()
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest,
                              new_opacities, new_scaling, new_rotation, factor_opt=None):
        d = {
            "xyz": new_xyz,
