...
PYBIND11_MODULE(pygicp, m) {
  ...
  py::class_<FastGICP, LsqRegistration, std::shared_ptr<FastGICP>>(m, "FastGICP")
    ...
    .def("update_correspondences", &FastGICP::update_correspondences_py)
    .def("get_final_error", &FastGICP::getFinalError)
    .def("set_num_threads", &FastGICP::setNumThreads)
    .def("set_correspondence_randomness", &FastGICP::setCorrespondenceRandomness)
    .def("set_k_choice_p2p", &FastGICP::setKChoiceP2P)
    ...
    .def("get_source_rotationsu", [] (FastGICP& gicp){
      std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> source_rotationsu = gicp.getSourceRotationsu();
      py::list data; for(const auto& q:source_rotationsu) data.append(q); return data;
      })
    .def("get_source_rotationsv", [] (FastGICP& gicp){
      std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> source_rotationsv = gicp.getSourceRotationsv();
      py::list data; for(const auto& q:source_rotationsv) data.append(q); return data;
      })
    .def("get_source_singular", [] (FastGICP& gicp){
      return py::array(gicp.getSourceScaleSize(), gicp.getSourceSingular().data());
      })
    ...
    .def("calculate_source_covs", &FastGICP::calculateSourceCovs)
    .def("get_source_raw_covs", [] (FastGICP& gicp){
      auto size = gicp.getSourceSize();
      const auto& raw_covs = gicp.getSourceRawCovariances();
      std::cout << "raw_covs size: " << raw_covs.size() << std::endl;
      py::array_t<double> numpy_raw_covs({size, 4, 4});
      auto buf = numpy_raw_covs.request();
      double* ptr = static_cast<double*>(buf.ptr);
      for(size_t i=0; i<size; ++i) {
        Eigen::Map<Eigen::Matrix<double, 4, 4>> (ptr + i*16) = raw_covs[i];
      }
      return numpy_raw_covs;
    })
    ...
    .def("get_source_correspondence", [] (FastGICP& gicp){
    	return py::make_tuple(py::array(gicp.getSourceSize(), gicp.getSourceCorrespondences().data()), 
            ...
            py::array(gicp.getSourceSize(), gicp.getSourcePoint2PlaneDistances().data()));
    })
    .def("set_source_covariances_fromqs", [] (FastGICP& gicp, py::array rotationsq, py::array scales, py::array rotationsu, py::array rotationsv, py::array singular){
      ...
      const auto input_rotationsu = rotationsu.cast<std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>>>();
      const auto input_rotationsv = rotationsv.cast<std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>>>();
      const auto input_singular = singular.cast<std::vector<float>>();

    	gicp.setSourceCovariances(input_rotationsq, input_scales, input_rotationsu, input_rotationsv, input_singular);
    })
