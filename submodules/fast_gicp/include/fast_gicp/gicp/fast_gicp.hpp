...
public:
  ...
  const std::vector<float>& getSourcePoint2PlaneDistances() const {
  	if (input_->size() != point2plane_dist_.size()){ std::cerr<< "source and point2plane_distances size mismatch. Did you change src after align()?"<<std::endl;}
  	return point2plane_dist_;}
  ...
  const std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d>>& getSourceRawCovariances() const {return source_raw_covs_;}
  ...
  const std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>>& getSourceRotationsu() const {return source_rotationsu_;}
  const std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>>& getSourceRotationsv() const {return source_rotationsv_;}
  const std::vector<float>& getSourceSingular() const {return source_singular_;}
  ...
  virtual void update_correspondences_py(const Matrix4& guess);
...
protected:
  ...
  std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d>> source_raw_covs_;
  std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> source_rotationsu_;
  std::vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>> source_rotationsv_;
  std::vector<float> source_singular_;
  ...
  std::vector<float> point2plane_dist_;
};
}  // namespace fast_gicp
